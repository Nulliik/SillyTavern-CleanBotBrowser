...
import { createCardBrowser, refreshCardGrid } from './ui/browser.js';
import { getOriginalMenuHTML, createBottomActions } from './ui/templates/templates.js';
import { SERVICE_ICON_URLS } from './ui/templates/serviceIcons.js';
import { escapeHTML, sanitizeImageUrl, secureRandomInt } from './utils/utils.js';
import { searchJannyCharacters, transformJannyCard, JANNYAI_TAGS } from './services/apis/jannyApi.js';
import { fetchJannyCollections, fetchJannyCollectionDetails } from './services/apis/jannyCollectionsApi.js';
import { createCollectionCardHTML, createCollectionsBrowserHeader } from './ui/templates/templates.js';
import {
...
} from './services/apis/backyardApi.js';
import './services/corsProxy.js';
import {
...
} from './services/apis/pygmalionApi.js';
import { initUpdateChecker } from './services/updateChecker.js';
import { searchRisuRealm, transformRisuRealmCard, resetRisuRealmState, risuRealmApiState, fetchRisuRealmTrending } from './services/apis/risuRealmApi.js';
...
import { fetchWyvernCreatorCards, searchWyvernCharacters, transformWyvernCard } from './services/apis/wyvernApi.js';
...
} from './services/apis/joylandApi.js';
import {
...
} from './services/apis/spicychatApi.js';
import {
...
} from './services/apis/talkieApi.js';
import {
...
function populateBookmarksTab(menu) {
    ...
    bookmarksGrid.innerHTML = bookmarks.map(card => `
        <div class="bot-browser-bookmark-card" data-card-id="${escapeHTML(card.id || '')}" data-nsfw="${card.possibleNsfw ? 'true' : 'false'}">
            <div class="bookmark-image" style="background-image: url('${sanitizeImageUrl(card.avatar_url || '')}');"></div>
        ...
        button.addEventListener('click', async () => {
            ...
            try {
                ...
                if (sourceName === 'all') {
                    ...
                    const { loadLocalLibrary } = await import('./services/localLibrary.js');
                    ...
                    const { loadLocalCharacters } = await import('./services/localLibrary.js');
                    ...
                    const { loadLocalLorebooks } = await import('./services/localLibrary.js');
...
// Standalone-style mode is kept in-page; the old iframe bridge stays disabled.
                ...
                <div class="bb-settings-body">
                    ...
                    <div class="bb-settings-tab-content active" data-content="filtering">
                        <div class="bb-setting-group">
                            ...
                            <textarea id="bb-setting-tag-blocklist" rows="5" placeholder="Enter tags or terms to block, one per line...">${escapeHTML((settings.tagBlocklist || []).join('\n'))}</textarea>
                    ...
                    <div class="bb-settings-tab-content" data-content="display">
                        ...
                        <div class="bb-setting-group">
                            <label>Cards Per Page: <span id="bb-cards-per-page-value">${escapeHTML(settings.cardsPerPage)}</span></label>
                            <input type="range" id="bb-setting-cards-per-page" min="50" max="500" step="50" value="${escapeHTML(settings.cardsPerPage)}">
                        </div>
                        ...
                        <div class="bb-setting-group">
                            <label>Max Recent Cards: <span id="bb-max-recent-value">${escapeHTML(settings.maxRecentlyViewed)}</span></label>
                            <input type="range" id="bb-setting-max-recent" min="5" max="20" value="${escapeHTML(settings.maxRecentlyViewed)}">
                        </div>
